...
class MessageService {
  Future<List<Conversation>> getConversations() async {
    ...
    if (uid == null) return [];
    final data = await supabase.rpc('get_user_conversations', params: {'p_user': uid});
  ...
  Future<String?> startConversation(String adId, String sellerId) async {
    final uid = supabase.auth.currentUser?.id;
    if (uid == null) return null;
    final conv = await supabase.rpc('start_conversation', params: {
      'p_ad_id': adId, 'p_buyer_id': uid, 'p_seller_id': sellerId
    });
